...
public class HeapPriorityQueue implements PriorityQueueInterface {
    ...
    public void priority(QueueElementInterface element) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (e.getValue().equals(element.getValue()) && element.getPriority().compareTo(e.getPriority()) < 0) {
    ...
    private void siftDown(int index) {
        ...
        while (index < size) {
            ...
            if (left < size && elements.get(left).getPriority().compareTo(elements.get(current).getPriority()) < 0) {
                current = left;
            }
            if (right < size && elements.get(right).getPriority().compareTo(elements.get(current).getPriority()) <0) {
                current = right;
            }
    ...
    private void siftUp(int index) {
        ...
        while (index >= 0) {
            ...
            if (parent >= 0 && elements.get(parent).getPriority().compareTo( elements.get(current).getPriority()) >0) {
                current = parent;
            }
